...
void AdaptersManager::setAdapterPowered(const Adapter *adapter, bool powered)
{
    if (!adapter)
        return;

    setAdapterPowered(adapter->id(), powered);
}

void AdaptersManager::setAdapterPowered(const QString &adapterId, bool powered)
{
    QDBusPendingCall call = m_bluetoothInter->SetAdapterPowered(QDBusObjectPath(adapterId), powered);
    ...
    if (!powered) {
        ...
        connect(watcher, &QDBusPendingCallWatcher::finished, [ = ] {
            if (clearUnpairedCall.isError())
                qCWarning(BLUETOOTH) << "Set adapter powered error: " << clearUnpairedCall.error().message();
        });
    }
}
...
void AdaptersManager::connectDevice(const Device *device, Adapter *adapter)
{
    if (device) {
        ...
        case Device::StateUnavailable: {
            ...
            qCDebug(BLUETOOTH) << "Connect to device: " << device->alias();
        }
            break;
        ...
        case Device::StateConnected: {
            ...
            qCDebug(BLUETOOTH) << "Disconnect device: " << device->alias();
        }
            break;
        }
    }
...
void AdaptersManager::adapterAdd(Adapter *adapter, const QJsonObject &adpterObj)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, adapter, call, watcher] {
        if (adapter) {
            if (!call.isError()) {
                ...
                qCWarning(BLUETOOTH) << "Add adapter error: " << call.error().message();
            }
        }
        delete watcher;
    });
